...
uniform float tone_mapping_gamme;
...
{
	vec3 color;
	vec3 texture_sample = texture(image, surface_texture_coord)
	color = texture_sample;
	//Espace gamma vers lineaire (eleve rgb a la puissance de tone_mapping)
	color = pow(color, vec3(tone_mapping_gamme))
	...
	//lineaire vers gamme
	color = pow(color, vec3(1.0 / tone_mapping_gamme));
